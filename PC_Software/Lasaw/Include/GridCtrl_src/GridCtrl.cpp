	...
	if (!GetVirtualMode())
	{
		if (nFixedRows > m_nFixedRows)
		{
			for (int i = m_nFixedRows; i < nFixedRows; i++)
				...
				{
					SetItemState(i, j, GetItemState(i, j) | GVIS_FIXED | GVIS_FIXEDROW | GVIS_READONLY);
    ...
    if (!GetVirtualMode())
    {
        if (nFixedCols > m_nFixedCols)
        {
            for (int i = 0; i < GetRowCount(); i++)
                ...
                {
					SetItemState(i, j, GetItemState(i, j) | GVIS_FIXED | GVIS_FIXEDCOL | GVIS_READONLY);
                    SetItemBkColour(i, j, CLR_DEFAULT );
                    SetItemFgColour(i, j, CLR_DEFAULT );
                }
        }
...
BOOL CGridCtrl::SetCellType(CCellRange Selection, CRuntimeClass* pRuntimeClass)
{
	if (GetVirtualMode())
		return FALSE;
	if (!pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(CGridCellBase)))
...
BOOL CGridCtrl::AddItemState(int nRow, int nCol, UINT state)
{
	...
	pCell->SetState(state|pCell->GetState());

	return TRUE;
}

BOOL CGridCtrl::AddItemState(CCellRange Selection, UINT state)
{
	for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
	{
		for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
...
BOOL CGridCtrl::SetItemState(int nRow, int nCol, UINT state, BYTE modify)
{
    BOOL bSelected = IsCellSelected(nRow, nCol);
...
BOOL CGridCtrl::SetItemState(CCellRange Selection, UINT state, BYTE modify)
{
	if (Selection.GetMaxRow() == -1)
		Selection.SetMaxRow(GetRowCount() - 1);
	if (Selection.GetMaxCol() == -1)
		Selection.SetMaxCol(GetColumnCount() - 1);
	for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
	{
		for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
		{
			...
			if (!IsValid(row, col))
				return FALSE;
			SetItemState(row, col, state,modify);
		}
	}
	return TRUE;
...
{
	TRACE0("CGridCtrl::OnRButtonDblClk\n");
	CCellID FocusCell;
	...
	if (!IsValid(FocusCell))
		SendMessageToParent(-1, -1, NM_RDBLCLK);
	else
	{
		SetFocusCell(-1, -1);
		...
		if (pCell)
			pCell->OnRDblClick(GetPointClicked(FocusCell.row, FocusCell.col, point));

		SendMessageToParent(FocusCell.row, FocusCell.col, NM_RDBLCLK);
	}
